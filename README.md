...
FilterFlow is not published on the Chrome Web Store. Google's third-party security validation program imposes prohibitive costs on independent developers. FilterFlow is open-source and self-hosted — each user supplies their own Google Cloud OAuth credentials, so all Gmail access goes directly through their own Google account.

## Prerequisites

- Node.js 18+
- A [Google Cloud](https://console.cloud.google.com/) project (free) with:
  - **Gmail API** enabled
...
4. **Load in Chrome (get your Extension ID)**

   - Navigate to `chrome://extensions/`
   - Enable **Developer mode**
   - Click **Load unpacked** and select the `dist/` folder
   - Note the **Extension ID** shown on the FilterFlow card
...
5. **Create Google Cloud credentials**
   ...
   - Choose **Chrome Extension** as the application type and enter your Extension ID
   - Copy the generated Client ID

6. **Configure OAuth**

   Paste your Client ID into `manifest.json` under `oauth2`:
...
Full usage guide, step-by-step installation instructions, and FAQ are available at the [FilterFlow documentation site](https://jamesshaver.github.io/FilterFlow/).
