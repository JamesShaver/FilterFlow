  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs font-medium text-slate-600 dark:text-slate-300">{t('dryRunPreview')}</span>
      ...
      <AnimatePresence mode="wait">
        {error ? (
          ...
          >
            <p className="text-xs text-slate-500 dark:text-slate-400">{t('matchingEmails', [String(messages.length), messages.length !== 1 ? 's' : ''])}</p>
            {messages.map((msg) => (
              <div key={msg.id} className="bg-white dark:bg-slate-700 rounded-md px-2.5 py-2 border border-slate-100 dark:border-slate-600">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 break-words mt-0.5">
                  {msg.snippet}
                </p>
              </div>
            ))}
          ...
          <div key="loading" className="space-y-1.5">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white dark:bg-slate-700 rounded-md px-2.5 py-2 border border-slate-100 dark:border-slate-600 animate-pulse">
                <div className="h-3 bg-slate-200 dark:bg-slate-600 rounded w-2/3 mb-1" />
                <div className="h-3 bg-slate-100 dark:bg-slate-650 rounded w-full" />
              </div>
            ))}
          </div>
        )}
